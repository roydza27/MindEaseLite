...
export default function MoodTracker({ onNavigate }: MoodTrackerProps) {
  ...
  const { t } = useLanguage();
  const { addMoodEntry, loading } = useDatabase();
  ...
  const handleNext = async () => {
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        console.log('Saving mood data to Firebase:', {
          mood: moodRating,
          stress: stressLevel,
        ...
        await addMoodEntry({
          mood: moodRating,
          stress: stressLevel,
          anxiety: parseInt(anxietyLevel) || 0,
          notes: `Mood: ${moodRating}/5, Stress: ${stressLevel}/5, Sleep: ${anxietyLevel}`
        });
        ...
        Alert.alert(
          'Error',
          'Failed to save your mood data. Please try again.',
          [{ text: 'OK' }]
        );
      }
    }
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.moodOptionButton,
                  {
                    ...
                    transform: [
                      { scale: moodRating === option.value ? 1.1 : 1 }
                    ],
                  }
                ]}
                onPress={() => {
                  console.log('Mood selected:', option.value, option.label);
                  setMoodRating(option.value as number);
                }}
                activeOpacity={0.8}
              >
                <Text style={[
