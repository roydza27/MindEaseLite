  ...
  const renderMoodSlider = () => {
    const panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        // Increase steps for smoother movement (0.1 increments)
        const newRating = Math.round((1 + (percentage * 4)) * 10) / 10;
      ...
      onPanResponderRelease: () => {
        // Snap to nearest 0.5 increment for better UX
        const snappedRating = Math.round(moodRating * 2) / 2;
        setMoodRating(snappedRating);
      },
    });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.background }]}>
        <TouchableOpacity style={[styles.backButton, { backgroundColor: colors.surface }]} onPress={handlePrevious}>
