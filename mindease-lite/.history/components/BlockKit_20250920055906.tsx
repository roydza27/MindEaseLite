...
export default function BlockKit({ onNavigate }: BlockKitProps) {
  ...
  useEffect(() => {
    if (isTimerActive) {
      ...
      // Start animations with a small delay to prevent conflicts
      pulseAnimation.start();
      setTimeout(() => rotateAnimation.start(), 100);
      setTimeout(() => glowAnimation.start(), 200);
      setTimeout(() => waveAnimation.start(), 300);
  ...
  const startTimer = () => {
    ...
    console.log('Timer started, isTimerActive:', true);
