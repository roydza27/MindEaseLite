  ...
  const renderMoodWheel = () => {
    return (
      <View style={styles.moodWheelContainer}>
        ...
        {/* Floating Mood Indicators in Circular/Oval Formation */}
        <View style={styles.moodIndicators}>
          {questions[0].options.map((option, index) => {
            // Create a more oval/circular distribution
            const angle = (index * 72) - 90; // 72 degrees per segment
            const radiusX = 160; // Horizontal radius for oval
            const radiusY = 120; // Vertical radius for oval
            ...
            return (
              <TouchableOpacity
                key={option.value}
                style={[
                  styles.floatingMoodIndicator,
                  {
                    left: width / 2 + x - 35,
                    top: 200 + y - 35,
                    backgroundColor: moodRating === option.value ? colors.primary : colors.surface,
                    borderColor: moodRating === option.value ? colors.primary : colors.border,
                    transform: [
                      { scale: moodRating === option.value ? 1.3 : 1 },
                      { translateY: moodRating === option.value ? -8 : 0 }
                    ],
                  }
                ]}
