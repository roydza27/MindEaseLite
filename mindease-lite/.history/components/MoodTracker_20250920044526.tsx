  ...
  const renderMoodArc = () => {
    const panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        const arcWidth = width - 100;
        const segmentWidth = arcWidth / 4;
        const newRating = Math.round((locationX - 50) / segmentWidth) + 1;
    ...
    return (
      <View style={styles.moodArcContainer} {...panResponder.panHandlers}>
  ...
  const renderSleepSlider = () => {
    const panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        const segmentHeight = trackHeight / 4;
        const newIndex = Math.round(locationY / segmentHeight);
        const options = ['excellent', 'good', 'fair', 'poor'];
        const clampedIndex = Math.max(0, Math.min(3, newIndex));
        setAnxietyLevel(options[clampedIndex]);
      },
      onPanResponderMove: (evt) => {
        const { locationY } = evt.nativeEvent;
        const trackHeight = 240;
    ...
    return (
      <View style={styles.sleepContainer}>
          ...
          {currentQ.options.map((option, index) => (
            <TouchableOpacity 
              ...
              style={[
                ...
                anxietyLevel === option.value && { backgroundColor: 'rgba(255, 255, 255, 0.1)' }
              ]}
        ...
        <View style={styles.sleepSlider} {...panResponder.panHandlers}>
